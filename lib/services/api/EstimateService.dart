...
class EstimateService {
  ...
  static saveEstimate(List<Module> modulesList) async {
    ...
    var response = await Api.call(new HttpRequest(
        HttpVerb.post, "estimates", jsonEncode(body), {"Content-Type": "application/json"}));
  }

  static getEstimates()async{
    var response = await Api.call(new HttpRequest(
        HttpVerb.get, "estimates", {}));
  ...
  static getEstimatesByCustomerId()async{
    var response = await Api.call(new HttpRequest(
        HttpVerb.get, "estimates", {"userId": customer.id}));
